...
type Situations = "Habitado" | "Habitável" | "Inabitavel" | "Inexplorado"
...
function salvePlanet(name:string, coordinates: PlanetCoordinates, situation:Situations ){
  planets.push({
    name, 
    coordinates,
    situation,
...
function updateSituation(situation: Situations, planet: Planet){
...
function promprValidPlanet(callback: (planet : Planet)=> void) {
  ...
  if(planet){
    ...
    alert('Planeta não encontrado ! Retornandp ao menu...')
  }
}
...
function firstMenuOption(){
  ...
  Coordenadasd : (${coordinateA}, ${coordinateB}, ${coordinateC}, ${coordinateD})
  Situação: ${situation}`)

  if(confirmation){
    salvePlanet(name, [coordinateA, coordinateB, coordinateC, coordinateD], situation)
  }
}

function secondMenuOption(){
  promprValidPlanet(planet=>{
    const situation = promptValidSituation()
...
function thirdMenuOption(){
  promprValidPlanet(planet=>{
    const satellite = prompt('Informe o nome do satélite a ser adcionado:')
